...
void deleteStudent(Student* records, int& count, bool isAdmin) {
    ...
    if (!isAdmin) {
        cout << "Only admins can delete records." << endl;
        return;
    }
    ...
    cout << "1. Soft delete" << endl;
    cout << "2. Hard delete" << endl;
    int deleteOption;
    cin >> deleteOption;
    ...
    for (int i = 0; i < count; ++i) {
        if (records[i].Id == idToDelete) {
            ...
            if (deleteOption == 1) {
                ...
                cout << "Invalid delete option. No action taken." << endl;
            }
            // Remove the deleted record from the active records
            count--;
            break;
        }
    }
...
void retrieveFromRecycleBin(Student* records, int& count) {
    ...
    if (recycleBin.is_open()) {
        Student student;
        int recordCount = 0;
        const int maxRecords = 100; // Maximum number of records in the recycle bin
        Student recycleBinRecords[maxRecords]; // Array to store records from recycle bin
        
        while (recordCount < maxRecords && recycleBin >> student.Id) {
            ...
            cout << "GPA Score: " << fixed << setprecision(2) << student.score << endl;

            // Add the retrieved record to the array
            recycleBinRecords[recordCount - 1] = student;
        }
        recycleBin.close(); // Close the recycle bin file after retrieval

        if (recordCount == 0) {
            cout << "No records found in the recycle bin." << endl;
            return;
        }

        // Ask user which record to retrieve
        int choice;
        cout << "\nEnter the number of the record to retrieve: ";
        cin >> choice;

        if (choice < 1 || choice > recordCount) {
            cout << "Invalid choice. No record retrieved." << endl;
            return;
        }

        // Retrieve the selected record
        Student retrievedRecord = recycleBinRecords[choice - 1];
        // Add the retrieved record to the active records
        records[count++] = retrievedRecord;

        // Remove the retrieved record from the recycle bin
        for (int i = choice - 1; i < recordCount - 1; ++i) {
            recycleBinRecords[i] = recycleBinRecords[i + 1];
        }

        // Rewrite the recycle bin file
        ofstream recycleBinOut(recycleBinFile);
        if (recycleBinOut.is_open()) {
            for (int i = 0; i < recordCount - 1; ++i) {
                recycleBinOut << recycleBinRecords[i].Id << "," << recycleBinRecords[i].name << ","
                              << recycleBinRecords[i].yearLevel << "," << recycleBinRecords[i].address << ","
                              << recycleBinRecords[i].birthDate << "," << recycleBinRecords[i].gender << ","
                              << fixed << setprecision(2) << recycleBinRecords[i].score << endl;
            }
            recycleBinOut.close();
            cout << "Record retrieved from recycle bin successfully." << endl;
        } else {
            cout << "Error: Unable to open recycle bin file for writing." << endl;
        }
    } else {
        cout << "Error: Unable to open recycle bin file." << endl;
    }
